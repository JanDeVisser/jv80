...
TEST_F(TESTNAME, outA) {
  outValue = 0x39;
  test_io(system, MOV_A_CONST, OUT_A);
  ASSERT_EQ(outValue, 0x42);
}

TEST_F(TESTNAME, outB) {
  outValue = 0x39;
  test_io(system, MOV_B_CONST, OUT_B);
  ASSERT_EQ(outValue, 0x42);
}

TEST_F(TESTNAME, outC) {
  outValue = 0x39;
  test_io(system, MOV_C_CONST, OUT_C);
  ASSERT_EQ(outValue, 0x42);
}

TEST_F(TESTNAME, outD) {
  outValue = 0x39;
  test_io(system, MOV_D_CONST, OUT_D);
  ASSERT_EQ(outValue, 0x42);
}

TEST_F(TESTNAME, inA) {
  inValue = 0x39;
  test_io(system, MOV_A_CONST, IN_A, CHANNEL_IN);
  ASSERT_EQ(gp_a -> getValue(), 0x39);
}

TEST_F(TESTNAME, inB) {
  inValue = 0x39;
  test_io(system, MOV_B_CONST, IN_B, CHANNEL_IN);
  ASSERT_EQ(gp_b -> getValue(), 0x39);
}

TEST_F(TESTNAME, inC) {
  inValue = 0x39;
  test_io(system, MOV_C_CONST, IN_C, CHANNEL_IN);
