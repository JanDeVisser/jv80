...
protected:
  ...
  void SetUp() override {
    mem = new Memory(0x0000, 0x2000, 0x8000, 0x2000, &image);
    (*mem)[0x0000] = 0x42;
    (*mem)[0x0001] = 0x37;
    (*mem)[0x0002] = 0x55;
...
TEST_F(MemoryTest, setMemAddressMSB) {
  mem -> setValue(0x5555);
  SystemError err = system -> cycle(false, true, 1, Memory::ADDR_ID, SystemBus::OP_MSB, 0x00, 0x00);
  ASSERT_EQ(err, NoError);
...
TEST_F(MemoryTest, setMemAddressLSBAndMSB) {
  ...
  SystemError err = system -> cycle(false, true, 1, Memory::ADDR_ID, 0, 0x01, 0x00);
  ASSERT_EQ(err, NoError);
  err = system -> cycle(false, true, 1, Memory::ADDR_ID, SystemBus::OP_MSB, 0x00, 0x00);
  ASSERT_EQ(err, NoError);
  ASSERT_EQ(mem -> getValue(), 0x0001);
}
